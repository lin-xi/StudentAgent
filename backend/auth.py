...
def generate_captcha_id() -> str:
    """生成验证码 ID。"""
    return hashlib.md5(f"{datetime.now(timezone.utc).isoformat()}{random.random()}".encode()).hexdigest()[:16]
...
def cleanup_expired_captchas():
    ...
    expired = [k for k, v in _captcha_store.items() if now - v['created'] > timedelta(minutes=5)]
    for k in expired:
...
def login(req: LoginRequest, response: Response):
    ...
    try:
        ...
        # 设置 cookie（3 个月有效期）
        session_token = hashlib.md5(f"{user['id']}{datetime.now(timezone.utc).isoformat()}".encode()).hexdigest()
